...
if msg.type == "pv" and redis:get(CZAR..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(CZAR..'sender:'..msg.sender_user_id_..':flood') then
...
if msg.Admin then redis:srem(CZAR..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(CZAR..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
for k, CZAR in pairs(ScriptFile.CZAR) do
local SearchText = msg.text:match(CZAR)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iCZAR(msg,{msg.text:match(CZAR)})
if GetMsg then
...
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) end,nil)   
	...
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/SourceCZAR/CZAR/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(CZAR..":SUDO_ID:")
redis:hset(CZAR..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(CZAR..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(CZAR..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(CZAR..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(CZAR..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(CZAR..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(CZAR..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(CZAR..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(CZAR..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(CZAR..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
